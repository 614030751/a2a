...
class ContinentalTireSupplyAgentExecutor(AgentExecutor):
    """马牌轮胎供应代理的执行器。"""

    def __init__(self):
        self.agent = ContinentalTireSupplyAgent()
    ...
    async def execute(
        ...
        async for item in self.agent.stream(query, task.context_id):
            ...
            await updater.add_artifact(
                [Part(root=TextPart(text=item['content']))], name='continental_supply_result'
            )
